...
export class DoctorComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if(window.localStorage.getItem('numberOfItemsOnPage') === null) {
     ...
     this.doctorManagerService.getNumberOfPatients().subscribe(result =>{this.numberOfPatients = result.body;
  ...
  deletePatient(patient: number) {
    this.patientManager.deletePatient(patient).subscribe(err => {err.body});
