...
export class DisplayVisitComponent implements OnInit {
  ...
  ngOnInit(): any {
    ...
    if (this.activatedRoute.snapshot.url[2]?.path == "displayVisit") {
      this.patientManager.getVisits(parseInt(this.activatedRoute.snapshot.url[3].path)).subscribe(err => {
      ...
      this.patientManager.getAbandonRequest().subscribe(err => {
        this.pending = err["body"]
        console.log(err);
      });

    } else if (this.activatedRoute.snapshot.url[0].path == "patient") {
      this.patientManager.getVisits(user.body.patient.id).subscribe(err => {
        this.pending = err["body"]["Oczekujace"]
        this.history = err["body"]["Historia"]


      });
    } else if (this.activatedRoute.snapshot.url[0].path == "doctor") {
